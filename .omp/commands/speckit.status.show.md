...
scripts:
  sh: .specify/extensions/status/scripts/bash/status.sh
  ps: .specify/extensions/status/scripts/powershell/status.ps1
---
...
1. **Run status script**: Execute `.specify/extensions/status/scripts/bash/status.sh` from the project root and parse the JSON output.
