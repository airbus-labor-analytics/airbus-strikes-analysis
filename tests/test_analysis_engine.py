...
class TestStrikeAnalysisEngine(unittest.TestCase):
    ...
    def test_salary_proposals_comparison_integrity(self):
        ...
        self.assertEqual(proposals["proposal-cgt"]["initial_increase_pct"], 14.0)
        self.assertEqual(proposals["proposal-cgt"]["arrears_lump_sum_eur"], 8500.0)
        self.assertEqual(proposals["proposal-strike-committee"]["initial_increase_pct"], 12.0)
        self.assertEqual(proposals["proposal-strike-committee"]["arrears_lump_sum_eur"], 7500.0)
        
        # Validate projections
        proj = res["projections"]
        self.assertEqual(len(proj["years"]), 6)
        co = proj["company"]
        cgt = proj["cgt"]
        ...
        self.assertGreater(com["total_5yr_nominal"], co["total_5yr_nominal"])
